      ...
      <table class="table table-hover mb-0 align-middle">
        ...
        <tbody>
          <tr v-if="!models.length">
            <td colspan="4" class="text-center p-3 text-muted">No vehicle models found.</td>
          </tr>
          <tr v-for="model in models" :key="model.id">
            <td>{{ model.modelname }}</td>
            <td>{{ model.odmeter_ioid || '-' }}</td>
            <td>{{ model.fuel_ioid || '-' }}</td>
            <td class="text-end">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form @submit.prevent="saveModel">
              ...
              <div class="mb-3">
                <label for="odometerIoid" class="form-label">Odometer IOID</label>
                <input type="text" class="form-control" id="odometerIoid" v-model="form.odmeter_ioid" placeholder="e.g. 238">
                <div class="form-text">IO ID used for odometer reading telemetry.</div>
              </div>
              <div class="mb-3">
                <label for="fuelIoid" class="form-label">Fuel IOID</label>
                <input type="text" class="form-control" id="fuelIoid" v-model="form.fuel_ioid" placeholder="e.g. 240">
                <div class="form-text">IO ID used for fuel level telemetry.</div>
              </div>
...
function openModal(model = null) {
  if (model) {
    ...
    form.odmeter_ioid = model.odmeter_ioid;
    form.fuel_ioid = model.fuel_ioid;
  } else {
